...
import { CURRENCIES, PHP_CASH, RIDER, RECENT_TXN, fmtRate, TOTAL_CAPITAL, OPENING_CAPITAL, CAPITAL_GAIN, TOTAL_STOCK_VALUE, TOTAL_UNREALIZED, TOTAL_THAN_TODAY, TOTAL_BOUGHT_TODAY, TOTAL_SOLD_TODAY } from '@/lib/data';
...
const CHART_DATA = [
  { t:'08:00', cap:1_000_000 }, { t:'09:30', cap:1_002_400 }, { t:'11:00', cap:1_005_800 },
  { t:'12:30', cap:1_008_200 }, { t:'14:00', cap:1_011_600 }, { t:'15:30', cap:1_018_900 },
  { t:'17:00', cap:1_023_400 }, { t:'18:43', cap:TOTAL_CAPITAL },
];
const PIE_DATA = [
  { name:'MAIN',     value: CURRENCIES.filter(c=>c.category==='MAIN').reduce((s,c)=>s+c.stockValuePhp,0),    color:'#00d4aa' },
  { name:'2ND',      value: CURRENCIES.filter(c=>c.category==='2ND').reduce((s,c)=>s+c.stockValuePhp,0),     color:'#5b8cff' },
  { name:'OTHERS',   value: CURRENCIES.filter(c=>c.category==='OTHERS').reduce((s,c)=>s+c.stockValuePhp,0),  color:'#f5a623' },
...
function Ticker() {
  const items = CURRENCIES.slice(0,9).flatMap(c => [`${c.flag} ${c.code}  B:${c.carryIn.rate.toFixed(c.decimalPlaces)}  S:${c.todaySellRate.toFixed(c.decimalPlaces)}`]);
  return (
    <div style={S.ticker}>
...
function DashboardTab() {
  const capital = useCountUp(TOTAL_CAPITAL, 1400);
  const gain = useCountUp(CAPITAL_GAIN, 1400);
  const than = useCountUp(TOTAL_THAN_TODAY, 1000);
  return (
    <div style={S.page}>
      {/* HERO */}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:24, alignItems:'center' }}>
          <div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:16, flexWrap:'wrap' }}>
              <span style={{ ...S.mono, fontSize:12, color:'#00d4aa', background:'rgba(0,212,170,0.1)', padding:'4px 12px', borderRadius:20, border:'1px solid rgba(0,212,170,0.2)' }}>+{php(gain)} vs opening ₱1M</span>
              <span style={{ ...S.mono, fontSize:12, color:'#4a5468' }}>+{((CAPITAL_GAIN/OPENING_CAPITAL)*100).toFixed(2)}% today</span>
            </div>
          </div>
          <div style={{ display:'flex', flexDirection:'column', gap:10, minWidth:190 }}>
            {[
              { label:'PHP CASH', val:PHP_CASH, color:'#a78bfa' },
              { label:'FX STOCK VALUE', val:TOTAL_STOCK_VALUE, color:'#5b8cff' },
              { label:'UNREALIZED GAIN', val:TOTAL_UNREALIZED, color:'#00d4aa', prefix:'+' },
            ].map(row => (
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14 }}>
        {[
          ...
          { label:'BOUGHT TODAY', val:php(TOTAL_BOUGHT_TODAY), sub:`${RECENT_TXN.filter(t=>t.type==='BUY').length} transactions`, color:'#5b8cff', icon:'💱', d:150 },
          { label:'SOLD TODAY', val:php(TOTAL_SOLD_TODAY), sub:`${RECENT_TXN.filter(t=>t.type==='SELL').length} transactions`, color:'#f5a623', icon:'💸', d:200 },
          { label:'RIDER IN FIELD', val:`₱${Math.round(RIDER.cashPHP/1000)}K`, sub:`${RIDER.transactions} txns · +${php(RIDER.marginPHP)} margin`, color:'#a78bfa', icon:'🏍️', d:250 },
        ].map(card => (
        ...
        <div style={{ ...S.card, padding:24, animation:'fadeUp 0.5s ease 0.3s both' }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={CHART_DATA} margin={{ top:4, right:4, bottom:0, left:0 }}>
              ...
              <YAxis domain={[995_000,1_035_000]} tick={{ fill:'#4a5468', fontSize:10, fontFamily:'DM Mono' }} axisLine={false} tickLine={false} tickFormatter={v=>`₱${(v/1000).toFixed(0)}K`} width={54}/>
        ...
        <div style={{ ...S.card, padding:24, animation:'fadeUp 0.5s ease 0.35s both' }}>
          ...
          <div style={{ display:'flex', justifyContent:'center' }}>
            <PieChart width={150} height={150}>
              <Pie data={PIE_DATA} cx={71} cy={71} innerRadius={44} outerRadius={68} dataKey="value" strokeWidth={0}>
      ...
      <div style={{ ...S.card, animation:'fadeUp 0.5s ease 0.4s both' }}>
        <div style={{ padding:'18px 24px', borderBottom:'1px solid #1e2230', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          ...
          <span style={{ ...S.mono, fontSize:11, color:'#00d4aa' }}>{RECENT_TXN.length} today</span>
        </div>
        {RECENT_TXN.slice(0,6).map((t,i) => (
...
function PositionsTab() {
  ...
  const filtered = filter==='ALL' ? CURRENCIES : CURRENCIES.filter(c=>c.category===filter);
  return (
    <div style={S.page}>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14 }}>
        {(['MAIN','2ND','OTHERS'] as const).map(cat => {
          const items = CURRENCIES.filter(c=>c.category===cat);
          return (
      ...
      <div style={S.card}>
        <div style={{ display:'grid', gridTemplateColumns:'130px 110px 120px 100px 100px 110px 100px 80px', padding:'10px 20px', borderBottom:'1px solid #1e2230', fontFamily:"'DM Mono',monospace", fontSize:10, color:'#4a5468', letterSpacing:'0.1em', gap:8 }}>
          <span>CURRENCY</span><span>STOCK QTY</span><span>AVG COST (PHP)</span><span>BUY RATE</span><span>SELL RATE</span><span>STOCK VALUE</span><span>UNREALIZED</span><span>RIDER</span>
        </div>
        {filtered.map((c,i) => (
          <div key={c.code} style={{ display:'grid', gridTemplateColumns:'130px 110px 120px 100px 100px 110px 100px 80px', padding:'13px 20px', borderBottom:i<filtered.length-1?'1px solid #1e2230':'none', background:i%2===0?'transparent':'rgba(255,255,255,0.012)', alignItems:'center', gap:8 }}>
            ...
            <span style={{ ...S.mono, fontSize:11, color:'#4a5468' }}>{fmtRate(c.dailyAvgCost, c.decimalPlaces)}</span>
            <span style={{ ...S.mono, fontSize:11 }}>{fmtRate(c.carryIn.rate, c.decimalPlaces)}</span>
            <span style={{ ...S.mono, fontSize:11, color:'#00d4aa' }}>{fmtRate(c.todaySellRate, c.decimalPlaces)}</span>
            ...
            <span style={{ ...S.mono, fontSize:11, color:c.riderQty?'#a78bfa':'#4a5468' }}>{c.riderQty?c.riderQty.toLocaleString():'—'}</span>
          </div>
        ))}
        <div style={{ display:'grid', gridTemplateColumns:'130px 110px 120px 100px 100px 110px 100px 80px', padding:'12px 20px', borderTop:'1px solid #1e2230', background:'rgba(167,139,250,0.04)', gap:8, alignItems:'center' }}>
          ...
          <span style={{ ...S.mono, fontSize:12, color:'#a78bfa', fontWeight:500 }}>{php(PHP_CASH)}</span>
          <span style={{ ...S.mono, fontSize:11, color:'#4a5468' }}>—</span>
          <span style={{ ...S.mono, fontSize:11, color:'#4a5468' }}>—</span>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'130px 110px 120px 100px 100px 110px 100px 80px', padding:'14px 20px', borderTop:'2px solid rgba(0,212,170,0.35)', background:'rgba(0,212,170,0.06)', gap:8, alignItems:'center' }}>
          <span style={{ ...S.syne, fontSize:13, fontWeight:800, color:'#00d4aa' }}>TOTAL</span>
          <span style={{ ...S.mono, fontSize:11, color:'#4a5468', gridColumn:'span 4' }}>{CURRENCIES.length} currencies + PHP cash</span>
          <span style={{ ...S.mono, fontSize:14, color:'#00d4aa', fontWeight:500 }}>{php(TOTAL_CAPITAL)}</span>
          <span style={{ ...S.mono, fontSize:12, color:'#00d4aa' }}>+{php(TOTAL_UNREALIZED)}</span>
          <span style={{ ...S.mono, fontSize:11, color:'#4a5468' }}>—</span>
        </div>
      </div>
    </div>
...
function TransactionsTab() {
  ...
  const filtered = RECENT_TXN.filter(t=>(tF==='ALL'||t.type===tF)&&(sF==='ALL'||t.source===sF));
  return (
    <div style={S.page}>
...
function RiderTab() {
  return (
    <div style={S.page}>
      <div style={{ ...S.card, border:'1px solid rgba(167,139,250,0.3)', padding:'28px 32px', animation:'fadeUp 0.4s ease both', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', top:-40, right:-40, width:150, height:150, borderRadius:'50%', background:'radial-gradient(circle, rgba(167,139,250,0.1) 0%, transparent 70%)', pointerEvents:'none' }} />
        <div style={{ display:'flex', gap:32, alignItems:'flex-start', flexWrap:'wrap' }}>
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
              <span style={{ fontSize:36 }}>🏍️</span>
              <div>
                <div style={{ ...S.syne, fontSize:20, fontWeight:800, color:'#a78bfa' }}>{RIDER.name}</div>
                <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:2 }}>
                  <div style={{ width:6, height:6, borderRadius:'50%', background:'#00d4aa', boxShadow:'0 0 6px #00d4aa88' }}/>
                  <span style={{ ...S.mono, fontSize:10, color:'#00d4aa', letterSpacing:'0.1em' }}>IN FIELD · Dispatched {RIDER.dispatchTime}</span>
                </div>
              </div>
            </div>
            <div style={{ ...S.mono, fontSize:11, color:'#4a5468', lineHeight:1.9 }}>
              {[['Transactions today', RIDER.transactions, '#e2e6f0'], ['Cash on hand', php(RIDER.cashPHP), '#a78bfa'], ['Total bought', php(RIDER.totalBoughtPHP), '#5b8cff'], ['Total sold', php(RIDER.totalSoldPHP), '#f5a623']].map(([k,v,c]) => (
                <div key={String(k)}>{k}: <span style={{ color: String(c) }}>{String(v)}</span></div>
              ))}
            </div>
          </div>
          <div style={{ display:'flex', gap:12, flex:1, minWidth:280, flexWrap:'wrap' }}>
            <div style={{ flex:1, background:'#161922', border:'1px solid #1e2230', borderRadius:12, padding:18, textAlign:'center', minWidth:120 }}>
              <div style={{ ...S.mono, fontSize:9, color:'#4a5468', marginBottom:8, letterSpacing:'0.12em' }}>MARGIN TODAY</div>
              <div style={{ ...S.syne, fontSize:22, fontWeight:800, color:'#00d4aa' }}>+{php(RIDER.marginPHP)}</div>
            </div>
            {RIDER.positions.map(p => (
              <div key={p.currency} style={{ flex:1, background:'#161922', border:'1px solid rgba(167,139,250,0.2)', borderRadius:12, padding:18, textAlign:'center', minWidth:100 }}>
                <div style={{ ...S.mono, fontSize:9, color:'#a78bfa', marginBottom:8, letterSpacing:'0.12em' }}>{p.currency} ON HAND</div>
                <div style={{ ...S.syne, fontSize:18, fontWeight:800 }}>{p.qty.toLocaleString()}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={S.card}>
        <div style={{ padding:'18px 24px', borderBottom:'1px solid #1e2230', ...S.syne, fontSize:14, fontWeight:700 }}>Rider Transactions Today</div>
        {RECENT_TXN.filter(t=>t.source==='RIDER').map((t,i,arr) => (
          <div key={t.id} style={{ display:'grid', gridTemplateColumns:'80px 58px 70px 100px 1fr 100px 90px', padding:'14px 24px', borderBottom:i<arr.length-1?'1px solid #1e2230':'none', alignItems:'center', gap:12 }}>
            <span style={{ ...S.mono, fontSize:10, color:'#4a5468' }}>{t.time}</span>
            <span style={{ ...S.mono, fontSize:10, textAlign:'center', padding:'2px 0', borderRadius:4, color:t.type==='BUY'?'#5b8cff':'#f5a623', background:t.type==='BUY'?'rgba(91,140,255,0.1)':'rgba(245,166,35,0.1)' }}>{t.type}</span>
            <span style={{ ...S.mono, fontSize:13, color:'#f5a623', fontWeight:500 }}>{t.currency}</span>
...
function RateBoardTab() {
  const today = new Date().toLocaleDateString('en-US', { weekday:'long', year:'numeric', month:'long', day:'numeric' });
  const half = Math.ceil(CURRENCIES.length / 2);
  const left  = CURRENCIES.slice(0, half);
  const right = CURRENCIES.slice(half);
  return (
    <div style={S.page}>
      {/* Header */}
          ...
          <div key={ci} style={S.card}>
            ...
            {col.map((c, i) => (
              <div key={c.code} style={{ display:'grid', gridTemplateColumns:'auto 1fr 110px 110px', padding:'13px 20px', borderBottom: i < col.length - 1 ? '1px solid #1e2230' : 'none', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.014)', gap:14, alignItems:'center' }}>
                ...
                <span style={{ ...S.mono, fontSize:15, fontWeight:600, color:'#5b8cff', textAlign:'right' }}>{fmtRate(c.carryIn.rate, c.decimalPlaces)}</span>
                <span style={{ ...S.mono, fontSize:15, fontWeight:700, color:'#00d4aa', textAlign:'right' }}>{fmtRate(c.todaySellRate, c.decimalPlaces)}</span>
              </div>
            ))}
          </div>
        ))}
