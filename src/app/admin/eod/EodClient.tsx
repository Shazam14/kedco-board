...
export default function EodClient({ role }: { role: 'admin' | 'supervisor' }) {
  const isTreasurer = role === 'supervisor';
  const hubHref     = isTreasurer ? '/supervisor' : '/admin';
  const hubLabel    = isTreasurer ? 'HUB' : 'Admin Home';
  const panelLabel  = isTreasurer ? 'Treasurer · EOD' : 'Admin Panel';
  const sectionTag  = isTreasurer ? 'TREASURER · EOD' : 'ADMIN · EOD';
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', color:'#e2e6f0' }}>
      ...
      <nav style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 32px', height:'56px', borderBottom:'1px solid var(--border)', background:'var(--nav-bg)', backdropFilter:'blur(12px)', position:'sticky', top:0, zIndex:100 }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            ...
            <div style={{ ...S.mono, fontSize:9, color:'var(--muted)', marginTop:-2 }}>{panelLabel}</div>
          </div>
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <a href="/dashboard" style={{ padding:'6px 16px', borderRadius:6, border:'1px solid var(--border)', background:'transparent', color:'var(--muted)', ...S.mono, fontSize:11, textDecoration:'none' }}>← Dashboard</a>
          <a href={hubHref} style={{ padding:'6px 16px', borderRadius:6, border:'1px solid rgba(91,140,255,0.3)', background:'rgba(91,140,255,0.08)', color:'#5b8cff', ...S.mono, fontSize:11, textDecoration:'none' }}>{hubLabel}</a>
        </div>
      </nav>
      ...
      <div style={{ padding:'28px 32px', maxWidth:680, margin:'0 auto', display:'flex', flexDirection:'column', gap:20 }}>
        ...
        {/* Header */}
        <div>
          <div style={{ ...S.mono, fontSize:10, color:'var(--muted)', letterSpacing:'0.2em', marginBottom:6 }}>{sectionTag}</div>
        ...
        {result && (
          <div style={{ ...S.card, padding:'28px', border:'1px solid rgba(0,212,170,0.3)', animation:'fadeUp 0.3s ease both' }}>
            ...
            <div style={{ display:'flex', gap:10, marginTop:16 }}>
              ...
              <a href={hubHref} style={{ flex:1, padding:'10px', borderRadius:8, border:'1px solid var(--border)', color:'var(--muted)', ...S.syne, fontSize:12, fontWeight:700, textDecoration:'none', textAlign:'center' }}>
                {hubLabel}
              </a>
            </div>
          </div>
