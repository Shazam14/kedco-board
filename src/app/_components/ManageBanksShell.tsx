  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080a10', color: '#e2e6f0' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', height: '56px', borderBottom: '1px solid #1e2230', background: 'rgba(15,17,23,0.96)', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ ...M, fontSize: 9, color: '#4a5468' }}>Manage Banks</div>
          </div>
        </div>
      ...
      <div style={{ padding: '28px 32px', maxWidth: 700 }}>
        <div style={{ ...M, fontSize: 10, color: '#4a5468', letterSpacing: '0.2em', marginBottom: 4 }}>ADMIN · BANKS</div>
        <div style={{ ...Y, fontSize: 24, fontWeight: 800, marginBottom: 24 }}>Manage Banks & E-wallets</div>

        {/* Add new */}
        <div style={{ background: '#0f1117', border: '1px solid rgba(0,212,170,0.2)', borderRadius: 14, padding: '20px 24px', marginBottom: 28 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 140px auto', gap: 10, alignItems: 'flex-start' }}>
            <div>
              <div style={{ ...M, fontSize: 9, color: '#4a5468', marginBottom: 4 }}>FULL NAME</div>
              <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g. Bank of Commerce"
                style={{ width: '100%', background: '#161922', border: '1px solid #1e2230', borderRadius: 8, padding: '10px 12px', color: '#e2e6f0', ...M, fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
            </div>
            <div>
              <div style={{ ...M, fontSize: 9, color: '#4a5468', marginBottom: 4 }}>CODE</div>
            ...
            <div style={{ paddingTop: 17 }}>
              <button onClick={addBank} disabled={saving || !newName.trim() || !newCode.trim()}
                style={{ padding: '10px 20px', borderRadius: 8, border: 'none', background: (!newName.trim() || !newCode.trim()) ? '#1e2230' : 'linear-gradient(135deg,#00d4aa,#00a884)', color: (!newName.trim() || !newCode.trim()) ? '#4a5468' : '#000', ...Y, fontSize: 12, fontWeight: 800, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                {saving ? '…' : '+ ADD'}
              </button>
            </div>
          </div>
        ...
        {/* Active list */}
        <div style={{ ...M, fontSize: 10, color: '#4a5468', letterSpacing: '0.12em', marginBottom: 10 }}>ACTIVE ({active.length})</div>
        <div style={{ background: '#0f1117', border: '1px solid #1e2230', borderRadius: 14, overflow: 'hidden', marginBottom: 24 }}>
          {active.map((b, i) => (
            <div key={b.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderBottom: i < active.length - 1 ? '1px solid #1e2230' : 'none', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.012)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {active.length === 0 && <div style={{ ...M, fontSize: 11, color: '#4a5468', padding: '16px 20px' }}>No active banks.</div>}
        </div>

        {/* Inactive list */}
        {inactive.length > 0 && (
          <>
            <div style={{ ...M, fontSize: 10, color: '#4a5468', letterSpacing: '0.12em', marginBottom: 10 }}>INACTIVE ({inactive.length})</div>
            <div style={{ background: '#0f1117', border: '1px solid #1e2230', borderRadius: 14, overflow: 'hidden' }}>
              {inactive.map((b, i) => (
                <div key={b.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderBottom: i < inactive.length - 1 ? '1px solid #1e2230' : 'none', opacity: 0.5 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <span style={{ ...M, fontSize: 11, color: '#4a5468', minWidth: 80 }}>{b.code}</span>
                    <span style={{ ...M, fontSize: 12, color: '#4a5468' }}>{b.name}</span>
                  </div>
                  <button onClick={() => toggleActive(b)}
