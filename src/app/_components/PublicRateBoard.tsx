    ...
    <div style={{ minHeight:'100vh', background:'var(--bg)', color:'#e2e6f0' }}>
      ...
      <nav style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 32px', height:'56px', borderBottom:'1px solid var(--border)', background:'var(--nav-bg)', backdropFilter:'blur(12px)', position:'sticky', top:0, zIndex:100 }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            ...
            <div style={{ ...S.mono, fontSize:9, color:'var(--muted)', marginTop:-2 }}>Pusok · Lapu-Lapu City</div>
          </div>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          {!isMobile && <span style={{ ...S.mono, fontSize:11, color:'var(--muted)' }}>{timeStr}</span>}
          {isLoggedIn
      ...
      <div style={{ padding: isMobile ? '16px' : '28px 32px', display:'flex', flexDirection:'column', gap:22 }}>
        ...
        <div style={{ ...S.card, padding:'24px 32px', display:'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent:'space-between', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? 16 : 0, border:'1px solid rgba(0,212,170,0.22)', animation:'fadeUp 0.3s ease both' }}>
          ...
          <div style={{ ...S.mono, fontSize:10, color:'var(--muted)', textAlign: isMobile ? 'left' : 'right', lineHeight:1.8 }}>
        ...
        {rates.length === 0 ? (
          <div style={{ ...S.card, padding:'48px', textAlign:'center' }}>
            ...
            <div style={{ ...S.mono, fontSize:11, color:'var(--muted)' }}>Please check back later or contact the counter.</div>
          </div>
        ) : (
          <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:14, animation:'fadeUp 0.4s ease 0.1s both' }}>
            {[left, right].map((col, ci) => (
              <div key={ci} style={S.card}>
                ...
                {col.map((r, i) => (
                  <div key={r.currency_code} style={{ display:'grid', gridTemplateColumns:'auto 1fr 110px 110px', padding:'13px 20px', borderBottom: i < col.length - 1 ? '1px solid var(--border)' : 'none', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.014)', gap:14, alignItems:'center' }}>
                    ...
                    <div>
                      ...
                      <span style={{ ...S.mono, fontSize:9, color:'var(--muted)', marginLeft:8 }}>{r.currency_code}</span>
                    </div>
